 ...
 *    메소드
 *    	구성요소
 *      처리결과 값(리턴값)            사용자 요청값(매개변수)
 *      예) 정수 두개를 보내주고 ==> 더한값을 달라
 *         --------매개변수       ----- 리턴값 
 ...
 *           
 *         = 더하기 기능
 *           int add(int a, int b)  가급적이면 int, a+b 맞추기
 *           ---
 *           {
 *            		//맨 마지막줄에
 *            		return a+b;
 ...
 *           
 *           void insert()
 *           void => 메소드 자체 처리 (데이터베이스 ==> insert, update, delete)
 *           {
 *           	return; // 생략 가능
 *           }
 *           
 ...
 *                         2) 숫자 (앞에 사용금지)
 *                    List<Emp> findByHiredate(int year) JPA
 *                         3) 특수문자 사용가능 ( _ , $ )
 *                         4) 키워드는 사용 금지
 *                         약속 사항 ===> 소문자로 시작한다.
 *                                      단어가 2개이상인 경우
 ...
 *                                  ----- 반드시 1개만 설정이 가능
 *           4. 매개변수
 *             ------- 1) 사용자가 입력 (명시적인 초기화, Scanner(입력값), random() )
 *                     2) 반복 제어 => 틀린문장
