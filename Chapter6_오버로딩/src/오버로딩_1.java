 ...
 *                                 ---------
 *                                 단점 : 폴더가 다른 경우에는 접근이 불가능
 *                    옵션 => static, final
 *      예)
 *          int a;     // 따로 저장  => 객체 생성 후 사용이 가능 (인스턴스명.변수명)
 *          static int b;   // 공유 => 자동 저장 => 클래스명으로 접근이 가능
 *          final int c=값;   // 상수
 *          -------------------------호출
 *      클래스 변수 : static
 ...
 *                 -----   ------
 *                  한개     여러개
 *               배열,클래스  Call By Value(메소드에서만 변경)
 ...
 *                      반드시 객체를 생성후에 접근이 가능하다
 *                      ----------------------------------데이터베이스 (static) 
 *   ----------------CBD
 *   오버로딩 : 생성자
 *     283page
 *     =======
 *     1) 하나의 메소드명으로 여러개의 기능을 만들 수 있다. (새로운 기능 추가)
 *     2) 메소드명을 줄일 수 있다
 *        ------------------ 기능이 동일
 *        println(int a)   ===> printInt()
 *        println(boolean)  ==> printBoolean()
 *        println(char[] c) ==>
 *        println(double d) ==>
 *        println(String s) ==>
 *        ...
