/*
 *     변수 : 한개의 데이터를 메모리에 저장하는 공강
 *     ------
 *         기본형 ( 자바에서 지원하는 메모리 크기) ==> 1byte, 2byte, 4byte, 8byte
 *           메모리크기 (컴퓨터는 무조건 0,1만 저장이 가능) => 0이나 1을 저장하는 공간 ==> bit
 *           ---> 8bit = 1byte
 *           
 *           정수
 *            byte ==> 1byte 크기의 메모리를 만들어서 저장이 가능하게 만들어 줌
 *                     --------------------------------------------
 *                      0  1  1  1  1  1  1  1  
 *                     --- 
 *                     부호비트 (0=>양수, 1=>음수)
 ...
 *        5) 변수의 길이는 제한하지 않는다 (3~7정도가 적당)
 *        ---------> 클래스명은 대문자 시작 String, System ...
 *                   변수나 메소드 소문자 시작
 *                   상수는 전체를 대문자 -----> 권장 ==> 분석이 쉽게 만든다 ==> 리팩토링(SM)
 *        => 변수 선언, 상수 선언
 *        변수선언 : 데이터형 변수명;    -------->선언 => 초기화, 선언과 동시에 초기화
 *        상수선언 : final 데이터형 변수명 ------> 선언과 동시에 초기화(변경할 수 없다)
 *        
 *        ***변수 선언 ex) 국어, 수학, 영어
 *           int kor;
 *           int math;
 ...
 *    자동 형변환
 *     => 초기값 대입
 *        long l=10; ==> 10L이 자동 저장
 *              ---- ->자동으로 long
 *        int a='A'; -> 'A'-> 65 int형으로 변환되어 저장
 *        int a=10.5; ==>저장 불가능, 오류
 *       ------
 *        왼쪽에 선언된 부분이 뒷값과 크거나 같아야함
 *        
 *        강제 형변환
 *           int a=(int)10.5; ==> a=10
 *           형변환 : boolean(제외)
 ...
 *      --------65+1=>66
 *      (int)(Math.random()*100)
 *       형변환 : (int)"10"  ==> 문자열은 형변환 불가능 (변경 ==> 라이브러리) : Wrapper 클래스
 ...
 *       byte+char=int
 *       
 *       데이터 저장 =====> 사용자 요청 처리 =======>결과값 (브라우저, 윈도우)
 *      ----------       --------------
 *                            연산자
 *  사용자 : id, pwd           id.equals("") && pwd.equals("")
