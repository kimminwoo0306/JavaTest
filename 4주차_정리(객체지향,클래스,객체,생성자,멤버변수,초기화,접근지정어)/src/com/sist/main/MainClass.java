 ...
 *                   -------- 상속 ---------------- 포함
 *             class My extends JFrame : 상속 (기존에 있는 클래스 확장 => 새로운 클래스 제작)
 *             class My
 *             {
 ...
 *                            ------ 클래스의 정보를 읽어서 메모리에 저장하는 방법
 *                      new를 사용하면 직접 연결이 있기 때문에 결합성 높은 프로그램
 *          구성 요소 ==> 무조건 포함 (생성자 => 없는 자동으로 추가 (생성자 : 디폴트 생성자 => 매개변수가 없는 생성자)
 *          멤버변수 : 메모리에 필요한 데이터를 저장할 목적 => 프로그램이 종료할 때 저장한다(클래스 전체에서 사용이 가능)
 *                   => 전역 변수
 *                1) 따로 저장 공간 (new) ------instance 변수
 *                2) 한개의 공간에만 저장 (모든 객체 공유) ----정적 변수 ( static 변수 )
 *                   => 한개의 회사
 *                      직원 정보 (직원마다 따로 저장)
 *                      회사명 (공유)
 *                   => CGV
 *                   => 인스턴스 변수 / static 변수(9:1)
 *                     => 데이터 여러개가 모여있다
 *                 형식)
 *                       private 데이터형 변수명
 ...
 *                  ---------------------- 메세지
 *                  리턴형 (화면 UI를 위해서 전송해야 되는 데이터) : 목록(배열) ==> 서버에서 처리 (Back)
 ...
 *                -------------------명시적인 초기화\
 *                  ==> 인스턴스 변수, static변수 ==> 초기화가 가능
 *                  초기화 블록
 *                  인스턴스 블록 : 인스턴스 변수, static 변수
 *                  static 블록 : static 변수만 가능
 *                  
 *                  class A
 *                  {
 *                  	private int a;
