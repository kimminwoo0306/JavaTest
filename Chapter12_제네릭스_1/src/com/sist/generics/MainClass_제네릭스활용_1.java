 ...
 *   => ArrayList, Vector, LinkedList(수정, 삭제가 많은 경우에 주로 사용)
 *      --------- Vector를 보완
 *      ---------------------------> 데이터베이스에 있는 데이터를 읽어와서 브라우저에 전송
 *                                   읽기 / 쓰기
 *                                   데이터베이스를 사용시에는 ArrayList에 있는 데이터는 거의 변경이 없다
 *                                   (수정, 삭제)
 *                                   => 접근성이 뛰어나다, 순차적으로 데이터 첨부시 속도가 빠르다
 * Set : 중복이 없는 데이터를 저장 할 경우 (중복을 허용하지 않는다), 순서가 없다
 ...
 *          HttpServletRequest HttpServeletResponse HttpSession Cookie
 *          클래스 저장, SQL문장 찾기
 *          --------   ----------
 *          Spring     MyBatis
 ...
 *   => 저장하는 데이터형을 명시한다
 *      형식)
 *          ArrayList<String> <사용자정의 데이터형도 첨부> <?> <? extends A>: A로부터 상속받은 클래스
 *          Set<String>
 *          Map(String, Object>
 */
import java.util.*;
