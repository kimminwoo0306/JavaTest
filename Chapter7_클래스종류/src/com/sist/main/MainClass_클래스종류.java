 ...
 *    3. 클래스 설계
 *       1) 사용자 정의 데이터형 ( 관련된데이터만 모아서 저장 ) => 데이터타입형 (캡슐화) => ~VO, ~DTO, ~Bean
 ...
 *   -------------------------------------------------------------------
 *    객체지향의 특성을 만든다 (유지보수가 편하게 만든다 => 권장사항)
 *      = 캡슐화
 ...
 *        포함 : 변경없이 사용 => 게시판 => 댓글
 *        상속 => 예외(초기화 블록, 생성자, static, private)
 *                형식)
 *                     class A extends B
 *                           -         -
 *                        하위클래스   상위클래스
 ...
 *          this    => 객체 자신 => 생성자, 멤버 메소드 에서만 사용이 가능 (static은 사용이 불가능)
 *                     this는 생략이 가능, 지역변수와 멤버변수가 동일할 때 구분시 많이 사용
 *                     다른 클래스로 현재 사용하고 잇는 객체를 전송 (윈도우=쓰레드 이용)
 *          this()  => 자신의 생성자 호출시에 사용 (생성자 => 첫줄에 사용)
 *          super   => 상속이 있는 경우 상위 클래스 객체
 *                     *** 상속을 받으면 하위클래스(상속을 받는 클래스) / 상위클래스(상속을 내리는 클래스) => 하위 클래스를 제어하지 못한다
 *                                       => 상위 클래스의 변수나 메소드를 제어
 *                         상위 클래스 : this
 *                         하위 클래스 : this, super
 ...
 *             {
 *             		Child c=new Child();  //  오류 발생 ==> 1) new Parent(), 2) new Child()
 *             }
 *             
 *             *** 자동 생성
 ...
 *           지역변수
 *             => 메소드 안에 선언 변수, 매개변수 => 메소드나 {}을 벗어나면 자동 삭제가 된다
 *             public void display()
 *             {
 *             		int a=10;
 *             		{
 ...
 *             [접근지정어][제어자] class Class Name
 *           * 메소드중에 abstract가 있는 경우에 private를 사용하면 안된다
 *           * 메소드중에 private, final을 동시에 사용할 수 없다.
 *           
...
abstract class 도형
{
	 ...
	 *      = 멤버 변수도 가지고 있다
	 *      = 사용시에는 반드시 메소드를 구현한 클래스를 이용해서 메모리를 할당
