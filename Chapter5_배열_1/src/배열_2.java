 ...
 *            stack / heap
 *            -----   ----
 *          메모리 주소  실제 데이터가 저장되는 공간 (메모리 ==> 주소값)
 *           데이터형 배열명[];  // c언어 호환
 *        
 *        예) 정수형 배열
 *           int[] arr;
 ...
 *           
 *           3) 배열이 메모리 배치되는 과정 ==> 인덱스 번호를 이용해서 관리
 *                                     ---------- 배열갯수-1 (0부터 시작, 초과되면 오류발생)
 */
public class 배열_2 {
	/*
	 *           변수
	...
	public static void main(String[] args) {
		...
		// 변수, 배열(변수의 일종) ==> 읽기(메모리에 저장된 데이터 가지고오기)/쓰기(저장, 수정해서 저장이 가능)
		// 값을 읽어오는 방법
		// 1. 4번째에 있는 데이터를 읽어온다 => 출력
		// 2. 5번째에 있는 데이터를 읽어온다 => 출력
		System.out.println("arr=>4번째 데이터: "+arr[3]);
		System.out.println("arr=>5번째 데이터: "+arr[4]);
		// 값을 변경하는 방법
		//1. 2번째에 있는 데이터를 200으로 변경
		//2. 3번째에 있는 데이터를 300으로 변경
		arr[1]=200;
		arr[2]=300;
